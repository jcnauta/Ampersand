   ...
   contents expr
    = let aVals = atomValuesOf ci ps 
      ...
      case expr of
         ...
         EMp1 _ _   -> Map.empty
         EBlt s sgn -> fatal 129 "Cannot populate a built-in relation without a context"
             -- for instance: _SESSION should contain the current session, but there is no current session
