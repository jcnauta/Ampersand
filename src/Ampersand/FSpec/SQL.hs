...
prettyQueryExprT :: Dialect -> QueryExpr -> SQLText
prettyQueryExprT dialect queryExpr = 
   toSQL . Text.pack $ prettyQueryExpr dialect queryExpr

broadQueryWithPlaceholder :: FSpec -> ObjectDef -> SQLText
broadQueryWithPlaceholder fSpec
  = unwordsT . wordsT 
     . prettyQueryExprT theDialect
     . broadQuery fSpec 
prettyBroadQueryWithPlaceholder :: Int -> FSpec -> ObjectDef -> SQLText
...
lengthCheck :: SQLText -> SQLText
lengthCheck str 
  | Text.length (toHaskellText str) <= maxLen = str
...
--  sqlQueryWithPlaceholder fSpec = lengthCheck . doNonPretty getBinQueryExprPlaceholder fSpec
  ...
  doNonPretty :: (FSpec -> a -> BinQueryExpr) -> FSpec -> a -> SQLText
  doNonPretty fun fSpec 
    =  unwordsT . wordsT 
     . prettyQueryExprT theDialect . bqeToSQL
     . stripComment
     . fun fSpec
  ...
  doPretty :: (FSpec -> a -> BinQueryExpr) -> Int -> FSpec -> a -> SQLText
  ...
  case expr of 
    EIsc (EDcI a , ECpl (ECps (EDcD r,EFlp (EDcD r')) )) 
      | r == r'   -> Just . BQEComment 
                              ...
                              , BlockComment $ "   "++showA expr++" ("++show (sign expr)++")"
                              ] $ 
    ...
    go isFlipped expr1 expr2 = Just .
      BQEComment
        [ BlockComment . unlines $
             ...
             , "  <expr2> = "++showA expr2++" ("++show (sign expr2)++")"
             , "   "++showA expr++" ("++show (sign expr)++")"
             ]
        ] $ BSE { bseSetQuantifier = SQDefault
                ...
                , bseTrg = Col { cTable = [table1]
                               ...
                               , cSpecial = Nothing}
                , bseTbl = [TRJoin 
                              (TRQueryExpr (bqeToSQL (selectExpr fSpec expr1)) `as` table1)
      ...
      (expr2Src,expr2trg,leftTable) =
         case expr2 of
           ...
           _ -> ( sourceAlias, targetAlias
                , TRQueryExpr (bqeToSQL (selectExpr fSpec (fun expr2))) `as` table2)
      table1 = Name "t1"
      table2 = Name "t2"
           ...
           where 
                  ...
                  f specificValue subTerms 
                     = BQEComment [BlockComment . unlines $
                                       ("case: (EIsc "++showA expr++" ("++show (sign expr)++")")
                                       :
                                       case expr of 
                                          EIsc (a,b) -> [show a, show b]
                                          _ -> fatal ("Not expecting anything else here than EIsc!\n  " ++ show expr)
                                  ] $
                        case subTerms of
                                         ...
                                         case x of
                                           ...
                                           _       -> BSE { bseSetQuantifier = bseSetQuantifier x
                                                          ...
                                                          , bseTrg = Col { cTable = [dummy]
                                                                         ...
                                                                         , cAlias = []
                                                                         , cSpecial = Nothing}
                                                          , bseTbl = [TRQueryExpr (bqeToSQL x) `as` dummy]
                                                          , bseWhr = Nothing
                                                          }
                                          ...
                                          es  -> -- Note: We now have at least two subexpressions
                                                 ...
                                                 BSE { bseSetQuantifier = SQDefault
                                                  ...
                                                  where
                                                   ...
                                                   tableRef n e = TRQueryExpr (bqeToSQL e) `as` iSect n
...
-}
                    ...
                    fenceTable i = 
                      ...
                      then makeNormalFence
                       ...
                       -- In some cases of a non-outer expression, a fence need not be generated, to get better SQL queries. 
                            ...
                            _      -> makeNormalFence
                     where
                       makeNormalFence = Just $ (TRQueryExpr . bqeToSQL . selectExpr fSpec) (fenceExpr i) `as` fenceName i
                        ...
                        makePole i 
                         = case (fenceTable i, fenceTable (i+1)) of
                             ...
                             (Nothing, Nothing) -> 
                                 ...
                                 Just . SubQueryExpr SqExists . bqeToSQL 
                                  . BQEComment [BlockComment "Case: ...;V[A*B];V[B*C];...."]
                                  . selectExpr fSpec . EDcI . target . fenceExpr $ i

                in BQEComment [BlockComment $ "case: (ECps es), with two or more elements in es."++showA expr]
                   BSE { bseSetQuantifier = SQDefault
    ...
    (ECpl e)
      -> case e of
           ...
           _             -> BQEComment (map BlockComment [ "case: ECpl e", "ECpl ( \""++showA e++"\" )"])
                            BSE { bseSetQuantifier = SQDefault
...
-- As a result of this way of working, code generated for =, |-, -, !, *, \, and / may not be efficient, but at least it is correct.
    EEqu (l,r)
      -> BQEComment [BlockComment $ "case: EEqu (l,r) "++showA expr++" ("++show (sign expr)++")"] $
         selectExpr fSpec ((ECpl l .\/. r) ./\. (ECpl r .\/. l))
    EInc (l,r)
      -> BQEComment [BlockComment $ "case: EInc (l,r) "++showA expr++" ("++show (sign expr)++")"] $
         selectExpr fSpec (ECpl l .\/. r)
    EDif (l,r)
      -> BQEComment [BlockComment $ "case: EDif (l,r) "++showA expr++" ("++show (sign expr)++")"] $
      ...
      -> let rResiduClause
              ...
              | otherwise
                  = BSE { bseSetQuantifier = SQDefault
                        ...
                        , bseWhr = Just . VEComment [BlockComment $ "Left hand side: "++showA l] $ 
                                          selectNotExists 
                                            (lCode `as` lhs)
                                            ( Just $ conjunctSQL
         ...
         in BQEComment [BlockComment $ "case: ERrs (l,r)"++showA expr++" ("++show (sign expr)++")"]
                         rResiduClause
    ELrs (l,r)
      -> BQEComment [BlockComment $ "case: ELrs (l,r)"++showA expr++" ("++show (sign expr)++")"] $
         selectExpr fSpec (EFlp (flp r .\. flp l))
    EDia (l,r)
      -> BQEComment [BlockComment $ "case: EDia (l,r)"++showA expr++" ("++show (sign expr)++")"] $
         selectExpr fSpec ((flp l .\. r) ./\. (l ./. flp r))
    ERad (l,ECpl r) 
      -> BQEComment [BlockComment $ "case: ERad (l, ECpl r)"++showA expr++" ("++show (sign expr)++")"] $
        selectExpr fSpec (EFlp (r .\. flp l))
    ERad (l,r) 
      -> BQEComment [BlockComment $ "case: ERad (l,r)"++showA expr++" ("++show (sign expr)++")"] $
        selectExpr fSpec (flp (notCpl l) .\. r)
    EPrd (l,r)
     -> let v = EDcV (Sign (target l) (source r))
        in BQEComment [BlockComment $ "case: EPrd (l,r)"++showA expr++" ("++show (sign expr)++")"] $
...
atomVal2InSQL val =
 case val of 
   AAVString{}         -> StringLit (Text.unpack . toHaskellText . safeSQL . toADLTxt $ val)
   ...
   _                   -> fatal ("Building a query with a literal `"
                                    ++show (aavtyp val)++"` is not implemented (yet?).")
toTableRef :: BinQueryExpr -> TableRef
toTableRef = TRQueryExpr . bqeToSQL
...
selectSorT att binExp =
     Select { qeSetQuantifier = SQDefault
            ...
            , qeFrom          = [TRQueryExpr (bqeToSQL binExp) `as` att]
            , qeWhere         = Nothing
            , qeGroupBy       = []
            , qeHaving        = Nothing
            , qeOrderBy       = []
...
stripCommentQueryExpr qe = 
   case qe of
     ...
     _               -> qe
bqeToSQL :: BinQueryExpr -> QueryExpr
bqeToSQL bqe 
 = case bqe of
    ...
    BCQE{} -> CombineQueryExpr 
                    { qe0 = bqeToSQL (bcqe0 bqe)
                    , qeCombOp = bcqeOper bqe
                    , qeSetQuantifier = bseSetQuantifier bqe
                    , qeCorresponding = Respectively  -- ??? What does this mean?
                    , qe1 = bqeToSQL (bcqe1 bqe)
                    }
    (BQEComment c (BQEComment c' e)) -> bqeToSQL $ BQEComment (c++c') e
    (BQEComment c e) -> QEComment c (bqeToSQL e)
...
sqlConceptPlug fSpec c 
             = case lookupCpt fSpec c of
                 []   ->  fatal ("A_Concept \""++show c++"\" does not occur in fSpec.")
                 (plug,_):_ -> plug
...
broadQuery fSpec obj = 
  ...
  -- 4) None of the above
  case objmsub obj of
   Nothing                -> bqeToSQL baseBinExpr
   Just InterfaceRef{}    -> bqeToSQL baseBinExpr
   Just Box{siObjs=sObjs} -> 
                    case filter (isInBroadQuery (objExpression obj)) sObjs of
                       [] -> bqeToSQL baseBinExpr
